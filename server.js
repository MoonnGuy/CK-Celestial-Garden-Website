...
async function sendStaffNotification(booking) {
  ...
  try {
    const result = await sendStaffEmailNotification(booking, message);
    console.log(`[email] ✓ Staff notification sent to ${CONFIG.staffEmail}`);
    return result;
  } catch (error) {
    console.error('[email] ✗ Staff notification failed:', error.message);
